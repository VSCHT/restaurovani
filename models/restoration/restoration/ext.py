...
from restoration import config
...
class RestorationExt:
    ...
    def published_service_records(self):
        from restoration.services.records.published.config import (
            RestorationPublishedServiceConfig,
        )
        from restoration.services.records.published.service import (
            RestorationPublishedService,
        )

        return RestorationPublishedService(
            config=RestorationPublishedServiceConfig(
    ...
    def published_service_files(self):
        from restoration.services.files.published.config import (
            RestorationFilePublishedServiceConfig,
        )
        from restoration.services.files.published.service import (
            RestorationFilePublishedService,
        )

        return RestorationFilePublishedService(
            config=RestorationFilePublishedServiceConfig(
